...
class AfterbuyClient:
    ...
    def __init__(self, partner_id: str, partner_token: str, account_token: str, 
                 user_id: str, user_password: str):
  ...
  <DataFilter>
    ...
    def _parse_order_response(self, xml_content: str) -> Optional[Dict]:
        ...
        try:
            ...
            call_status = root.find('CallStatus').text if root.find('CallStatus') is not None else None
    ...
    def parse_memo(self, memo_text: str) -> Dict:
        ...
        for i, line in enumerate(lines):
            ...
            # Check if line is an amount (e.g., "1.680,00 EUR" or "1,680.00 EUR")
                ...
                amount_clean = line.replace('.', '').replace(',', '.').replace('EUR', '').strip()
                try:
...
def create_client_from_config(config):
    ...
    return AfterbuyClient(
        ...
        partner_token=os.getenv('AFTERBUY_PARTNER_TOKEN', '6722d455-4d02-4da3-97ef-f5dfcf73656d'),
        account_token=os.getenv('AFTERBUY_ACCOUNT_TOKEN', '53217733-1987-4cf8-a065-2c2591e4765c'),
