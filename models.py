...
class Call(db.Model):
    ...
    status = db.Column(db.Enum(CallStatus), nullable=False, default=CallStatus.PROCESSING)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationships
    conversations = db.relationship('Conversation', backref='call', lazy=True, cascade='all, delete-orphan')
...
class Conversation(db.Model):
    ...
    step = db.Column(db.String(50), nullable=False)  # greeting, consent, order, help, etc.
